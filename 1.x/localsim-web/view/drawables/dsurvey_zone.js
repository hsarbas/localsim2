class DSurveyZone{
    ...
    move(){
        ...
        if(this.road_type == 'Uninterrupted_Road'){
            points = this._get_zone_points();
        }
        else{
            points = this._get_iroad_points();
        }
    ...
    _get_zone_points(){
        ...
        let d = (this.object.road.width / 2.0);
        ...
        for(let segment of this.segments){
            ...
            if(segment != start_seg && segment != end_seg){
                ...
                temp = new Map();
                temp.set('x', in_src.get('x'));
                temp.set('y', in_src.get('y'));
                points.push(temp);

                temp = new Map();
                temp.set('x', in_dst.get('x'));
                temp.set('y', in_dst.get('y'));
                points.push(temp);
            }
        ...
        for(let segment of reversed){
            d = (this.object.road.width / 2.0) - (this.object.road.get_lanes() * this.object.road.get_lane_width());
            ...
            if(segment != start_seg && segment != end_seg){
                ...
                ret = delta_pt_in_perp_line(in_src.get('x'), in_src.get('y'), in_dst.get('x'), in_dst.get('y'),
                    this.object.road.get_lanes() * this.object.road.get_lane_width());
                let dx = ret[0];
                let dy = ret[1];
